...
setup_previews() {
    ...
    if [ $should_update -eq 1 ] || [ ! -d "$CONFIG_DIR/previews" ] || [ -z "$(find "$CONFIG_DIR/previews" -name "*.png" 2>/dev/null)" ]; then
        ...
        if ! download_and_display_progress_unknown_size "$PREVIEW_PACK_URL" "$CONFIG_DIR/theme_previews.7z" "Theme Previews"; then
...
log_and_display_message "Welcome to the sprigUI Theme Garden. We hope you enjoy your visit as you stop and smell the artwork. Please wait.........."
